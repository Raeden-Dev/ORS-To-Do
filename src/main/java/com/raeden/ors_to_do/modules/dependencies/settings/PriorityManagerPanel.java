...
public class PriorityManagerPanel extends VBox {
    ...
    private void renderExistingPriorities() {
        ...
        for (int i = 0; i < appStats.getCustomPriorities().size(); i++) {
            TaskItem.CustomPriority prio = appStats.getCustomPriorities().get(i);
            int index = i;
            ...
            editBtn.setOnAction(e -> {
                ...
                dialog.showAndWait().ifPresent(res -> {
                    if (res == ButtonType.OK && !nameF.getText().trim().isEmpty()) {
                        appStats.getCustomPriorities().set(index, new TaskItem.CustomPriority(nameF.getText().trim(), toHexString(colC.getValue())));
                        StorageManager.saveStats(appStats);
                        renderExistingPriorities();
                        refreshCallback.run();
                    }
