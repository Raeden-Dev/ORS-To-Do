...
public class StatsManagerPanel extends VBox {
    ...
    public StatsManagerPanel(AppStats appStats, Runnable refreshCallback) {
        ...
        Label desc = new Label("Create custom stats (Strength, Focus, etc.) to attach to your tasks.");
        ...
        getChildren().addAll(header, desc, existingStatsBox, new Separator(), createStatBtn);

        // --- NEW: Disable entirely if Global Setting is OFF ---
        if (!appStats.isGlobalStatsEnabled()) {
            this.setDisable(true);
            desc.setText("⚠️ Custom Stats are disabled. Turn them on in General Configuration to use this feature.");
            desc.setStyle("-fx-text-fill: #FF6666; -fx-font-weight: bold; -fx-font-size: 12px;");
        }
    }
